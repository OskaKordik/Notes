...
class  LexisUseCaseImpl(private val api: MainApi,private val prefs: SettingsPrefs,private val dao: LexicDao, private val context: Context): LexisUseCase  {
    override suspend fun execute(vararg ids: Int): List<Lexic> {
        val fromDb = dao.getLexics(ids.asList().map {
            ...
            context.resources.getInteger(it) },prefs.getLanguage())
        if (fromDb.size != ids.size){
           val fromServer = api.getTranslate(BaseRequest(procedure = API_TRANSLATE,params = TranslateRequest(
